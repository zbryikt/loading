...
angular.module \main, <[uiloading colorpicker.module]>
  ...
  ..controller \main, <[$scope $injector $timeout $interval $http $compile capture outputmodal]> ++ ($scope, $injector, $timeout, $interval, $http, $compile, capture, outputmodal) ->
    ...
    $scope.build = do
      choices: <[default infinity ellipsis dashinfinity reload wheel g0v pacman facebook spin ball cube circle pie radio poi gear gears comment wave battery sunny triangle flickr ring]>
      anitimer: null
      size: 60
      running: true
      making: false
